...
The generic Locus V2 × Locus V2 rich-only boundary is not broadened. For the
published structural SplineV2 pair subset, R1 now provides certified local
existence/uniqueness, transversality and singleton-germ semantic selectors.
Only exact currently admissible R1 tokens may enter the materialization UI.
...
Optional auto-materialization is an explicit, visible frontend operation
immediately after the rich query is created, inside the same user-confirmed
compound undo transaction. One Enter/tool confirmation publishes the query
and opted-in points with one undo store; subsequent explicit inspector
materialization remains its own atomic action. Kernel recompute,
