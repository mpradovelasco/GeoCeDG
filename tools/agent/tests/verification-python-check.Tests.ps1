...
C 'producer resolves registered cedg_env and invokes its Python directly' {
    ...
    A($source-match'Resolve-VerificationCedgEnvironment') 'portable environment resolver is not used'
    A(-not($source-match'\.conda[/\\]environments\.txt')) 'volatile Conda inventory is consulted'
...
C 'unwritable Conda package storage cannot affect direct interpreter execution' {
    ...
    A($source-match'environment_resolved\s*=') 'portable resolution evidence is absent'
    A($source-match'environment_resolution_source\s*=') 'resolution source evidence is absent'
}
C 'exact interpreter and import provenance is required before scientific exit is interpreted' {
    ...
    $evidence=[pscustomobject]@{completion_state='COMPLETED';inner_exit_code=0;identity_exit_code=0;identity_state='PRESENT';environment_name='cedg_env';environment_prefix=$prefix;environment_resolution_source='CONFIGURED_ENV_ROOT';environment_resolved=$true;command=$identity.executable;identity=$identity}
    ...
    $evidence.environment_resolved=$false
    A((ConvertFrom-VerificationPythonCheckEvidence $evidence SCIENTIFIC).outcome -ceq 'EVIDENCE_UNTRUSTED') 'unresolved prefix was trusted'
}
