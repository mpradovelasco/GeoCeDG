...
Invoke-Case 'tracked JUnit inventory is compact and selection based' {
    ...
    Assert-Case (@($inventory.selections).Count -eq 11) 'Selection inventory count changed.'
...
Invoke-Case 'all 29 PHASE selections resolve as complete pure plans' {
    Assert-Case (@($registry.phase_selections).Count -eq 29) 'PHASE selection count changed.'
