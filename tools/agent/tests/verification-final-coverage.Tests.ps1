...
Invoke-Case 'tracked JUnit inventory is compact and selection based' {
    ...
    Assert-Case (@($inventory.selections).Count -eq 17) 'Selection inventory count changed.'
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a7.desktop')[0].expected_identity_count -eq 54) `
...
Invoke-Case 'all 32 PHASE selections resolve as complete pure plans' {
    Assert-Case (@($registry.phase_selections).Count -eq 32) 'PHASE selection count changed.'
