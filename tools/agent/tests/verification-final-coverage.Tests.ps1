...
Invoke-Case 'tracked JUnit inventory is compact and selection based' {
    ...
    Assert-Case (@($inventory.selections).Count -eq 27) 'Selection inventory count changed.'
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a6.narrow')[0].expected_identity_count -eq 8) `
        'POST-G9U1-A6 inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a1.narrow')[0].expected_identity_count -eq 117) `
        'POST-G9U1-A1 inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a2.narrow')[0].expected_identity_count -eq 39) `
        'POST-G9U1-A2 inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a3.narrow')[0].expected_identity_count -eq 114) `
        'POST-G9U1-A3 inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a3-frontend.shared')[0].expected_identity_count -eq 3) `
        'POST-G9U1-A3-FRONTEND shared inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'post-g9u1-a7.shared')[0].expected_identity_count -eq 2) `
        'POST-G9U1-A7 shared inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-s1.desktop')[0].expected_identity_count -eq 150) `
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-s1-r2.desktop')[0].expected_identity_count -eq 36) `
        'PRE-G9B-S1-R2 Desktop inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-s2.shared')[0].expected_identity_count -eq 11) `
        'PRE-G9B-S2 shared inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
    ...
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-s4.desktop')[0].expected_identity_count -eq 12) `
        'PRE-G9B-S4 Desktop inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-p0.desktop')[0].expected_identity_count -eq 35) `
        'PRE-G9B-P0 Desktop inventory changed.'
    Assert-Case (@($inventory.selections|Where-Object selection_id -CEQ `
        'pre-g9b-p1.desktop')[0].expected_identity_count -eq 36) `
...
Invoke-Case 'all 39 PHASE selections resolve as complete pure plans' {
    Assert-Case (@($registry.phase_selections).Count -eq 39) 'PHASE selection count changed.'
