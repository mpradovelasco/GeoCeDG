...
Invoke-Case 'tracked JUnit inventory is compact and selection based' {
    ...
    Assert-Case (@($inventory.selections).Count -eq 26) 'Selection inventory count changed.'
...
Invoke-Case 'all 38 PHASE selections resolve as complete pure plans' {
    Assert-Case (@($registry.phase_selections).Count -eq 38) 'PHASE selection count changed.'
