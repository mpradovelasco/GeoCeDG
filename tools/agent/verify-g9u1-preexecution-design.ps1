...
$ExpectedBranch = "codex/g9u1-construction-workspace-after-r1"
$PlanningBaseCommit = "f8a21a087234b18fc13741a0ac2baf80608e9022"
$R1PassCommit = "af459d856f1cdc384805f3035203acce8e6f6104"
$R1PassTagObject = "2b8caf1d0628be568c9afb062628d79d8f6a1342"
...
$ProtectedPostR6Branch = "feature/g9u1-construction-workspace-planning-after-r6"
$ProtectedPostR6Commit = "00982e7e148a634cd57ed928f322774df267d5e3"
$ProtectedPostR6PromptHash =
...
function Assert-ApprovedEntryAuthority {
    ...
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
            $head -eq $PlanningBaseCommit) `
        -Message "Post-R1 planning does not start at the published operational base."
    ...
    $r1TagRef = (& git -C $RepositoryRoot rev-parse `
        "refs/tags/geocedg-g9s1-r1-pass").Trim()
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
            $r1TagRef -eq $R1PassTagObject) `
        -Message "The published R1 annotated tag object changed."
    $r1TagType = (& git -C $RepositoryRoot cat-file -t $R1PassTagObject).Trim()
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and $r1TagType -eq "tag") `
        -Message "The published R1 tag is not annotated."
    $r1TagPeel = (& git -C $RepositoryRoot rev-parse "$R1PassTagObject^{}").Trim()
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
            $r1TagPeel -eq $R1PassCommit) `
        -Message "R1 PASS must identify its phase closeout, not the operational successor."
    & git -C $RepositoryRoot merge-base --is-ancestor $R6PassCommit $R1PassCommit
    Assert-Condition -Condition ($LASTEXITCODE -eq 0) `
        -Message "R6 is not ancestral to the approved R1 closeout."
    & git -C $RepositoryRoot merge-base --is-ancestor $R1PassCommit $PlanningBaseCommit
    Assert-Condition -Condition ($LASTEXITCODE -eq 0) `
        -Message "R1 is not ancestral to the planning base."
    foreach ($productRef in @("main", "origin/main")) {
        $productHead = (& git -C $RepositoryRoot rev-parse $productRef).Trim()
        Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
                $productHead -eq $PlanningBaseCommit) `
            -Message "Published product authority changed: $productRef."
    }
    ...
    foreach ($planningRef in @($ProtectedPostR6Branch, "origin/$ProtectedPostR6Branch")) {
        $planningCommit = (& git -C $RepositoryRoot rev-parse $planningRef).Trim()
        Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
                $planningCommit -eq $ProtectedPostR6Commit) `
            -Message "Protected approved post-R6 planning ref changed: $planningRef."
    }
    Assert-Condition -Condition ((Get-CanonicalLfSha256 $PostR6PromptPath) -eq
            $ProtectedPostR6PromptHash) `
        -Message "The approved historical post-R6 canonical prompt changed."
}
...
function Add-U1IdsFromJsonNode {
    ...
    if ($Node -is [string]) {
        if ($Node -match '^U1-(?:(?:R5|S1|PNT|PAIR)-\d{2}|(?:S|W|D|L|V|I|B|A|G|C|Q|P)\d{2})$') {
            $Result.Add($Node)
        }
        return
    }
...
function Assert-ScenarioInventory {
    ...
    Assert-Condition -Condition ($scenarioIds.Count -eq $ExpectedScenarioCount) `
        -Message ("Expected 138 unique U1 scenario IDs in JSON; found " +
            "$($scenarioIds.Count).")

    $matrixText = Get-RequiredText -RelativePath $PublicMatrixPath
    $matrixIds = @([regex]::Matches(
            $matrixText, '(?m)^\|\s*(?<id>U1-(?:(?:R5|S1|PNT|PAIR)-\d{2}|(?:S|W|D|L|V|I|B|A|G|C|Q|P)\d{2}))\s*\|') |
    ...
    Assert-Condition -Condition ($matrixUnique.Count -eq $ExpectedScenarioCount) `
        -Message ("Expected 138 unique U1 public-matrix rows; found " +
            "$($matrixUnique.Count).")
    ...
    foreach ($group in $ExpectedScenarioGroups.GetEnumerator()) {
        $suffix = if ($group.Key -in @("U1-R5", "U1-S1", "U1-PNT", "U1-PAIR")) {
            '-\d{2}$'
        } else {
            '\d{2}$'
        }
...
function Assert-EvidenceIntegrity {
    ...
    Assert-Condition -Condition ($evidenceText.Contains($ProtectedPostR6PromptHash) -and
            $reportText.Contains($ProtectedPostR6PromptHash)) `
        -Message "The approved post-R6 prompt hash is not preserved."
...
function Assert-PlanningDisposition {
    ...
    $evidence = Get-RequiredJson -RelativePath $EvidencePath
    Assert-Condition -Condition (
            -not [bool]$evidence.approval.implementationStarted -and
            [bool]$evidence.approval.implementationAuthorized -and
            -not [bool]$evidence.approval.selfApproved -and
            [bool]$evidence.approval.authorApprovedDesign -and
            -not [bool]$evidence.approval.passClaimedImplementation) `
        -Message "Planning must record conditional implementation authorization, not execution or approval."
    ...
    Assert-Contains -RelativePath $CompletenessMatrixPath -RequiredText @(
        "ZoomWindow")
    Assert-Contains -RelativePath $PromptPath -RequiredText @(
        "pair-singleton-transverse-germ/v1", "rich-only", "singleton", "owner")
    foreach ($path in @(
    ...
    Assert-NotContains -RelativePath $PromptPath -ForbiddenText @(
        "G9U1 = PASS — AUTHOR APPROVED", "authorApprovedImplementation = true",
    ...
    # Planning references must resolve to the actual published APIs, not a
    # provisional interface inferred from earlier design prose.
    $operations = "source/shared/common/src/main/java/org/geocedg/common/kernel/locus/LocusV2PublicOperations.java"
    Assert-Contains -RelativePath $operations -RequiredText @(
        "createInteractiveSemanticPoint(", "moveInteractiveSemanticPoint(",
        "selectIntersectionPoint(")
    Assert-Contains -RelativePath `
        ...
        -RequiredText @("isPointAdmissible(", "findExactPointAdmissibleSolution(")
    Assert-Contains -RelativePath `
        "source/shared/common-jre/src/test/java/org/geocedg/common/locus/G9S1R1SplinePairMaterializationTest.java" `
        -RequiredText @("reversedCallerOperandsHaveEquivalentCurrentSlotEvidence")
}
...
try {
    ...
    $summary = @(
        ...
        "R1 authority: geocedg-g9s1-r1-pass -> $R1PassCommit.",
        "Published operational base: $PlanningBaseCommit.",
        "Protected pre-R6 planning checkpoint: $ProtectedPlanningCommit.",
        "Protected approved post-R6 planning checkpoint: $ProtectedPostR6Commit.",
        ...
        "G9S1-R1 and G9U0-R6 are PASS — AUTHOR APPROVED; G9U1 design is PASS — AUTHOR APPROVED; post-R1 reconciliation found no material novelty; conditional implementation is authorized but unexecuted at this planning freeze."
    )
