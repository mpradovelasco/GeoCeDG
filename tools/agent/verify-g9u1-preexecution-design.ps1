...
$ExpectedBranch = "feature/g9u1-construction-workspace-planning"
...
function Assert-PlanningScope {
    ...
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
        ...
        -Message "The live schema-v1 profile authority is not the sealed G9S1 blob."
...
function Assert-ApprovedEntryAuthority {
    ...
    Assert-Condition -Condition ($LASTEXITCODE -eq 0 -and
            $head -eq $G9S1PassCommit) `
        -Message "G9U1 planning does not start at the approved G9S1 commit."
...
function Add-U1IdsFromJsonNode {
    ...
    if ($Node -is [string]) {
        if ($Node -match '^U1-(?:(?:R5|S1)-\d{2}|(?:S|W|D|L|V|I|B|A|G|C|Q|P)\d{2})$') {
            $Result.Add($Node)
        }
        return
    }
...
function Assert-ScenarioInventory {
    ...
    Assert-Condition -Condition ($scenarioIds.Count -eq $ExpectedScenarioCount) `
        -Message ("Expected 98 unique U1 scenario IDs in JSON; found " +
            "$($scenarioIds.Count).")

    $matrixText = Get-RequiredText -RelativePath $PublicMatrixPath
    $matrixIds = @([regex]::Matches(
            $matrixText, '(?m)^\|\s*(?<id>U1-(?:(?:R5|S1)-\d{2}|(?:S|W|D|L|V|I|B|A|G|C|Q|P)\d{2}))\s*\|') |
    ...
    Assert-Condition -Condition ($matrixUnique.Count -eq $ExpectedScenarioCount) `
        -Message ("Expected 98 unique U1 public-matrix rows; found " +
            "$($matrixUnique.Count).")
    ...
    foreach ($group in $ExpectedScenarioGroups.GetEnumerator()) {
        $suffix = if ($group.Key -in @("U1-R5", "U1-S1")) {
            '-\d{2}$'
        } else {
            '\d{2}$'
        }
...
function Assert-EvidenceIntegrity {
    ...
    foreach ($requiredHashPath in @(
            $PromptPath, $ScenarioPath, $EvidencePath, $ReportPath)) {
    ...
    Assert-Condition -Condition ($validated -ge 4) `
        -Message "The G9U1 evidence manifest is unexpectedly incomplete."
    return [pscustomobject]@{
...
function Assert-PlanningDisposition {
    Assert-Contains -RelativePath $PromptPath -RequiredText @(
        "DEFINITIVE DESIGN CANDIDATE",
        "PENDING AUTHOR REVIEW / UNEXECUTED / NOT AUTHORIZED",
        "G9U0-R6 — SEMANTIC LOCUS POINT INTERACTION SUPPORT",
        "PASS — AUTHOR APPROVED",
        "This prompt neither",
        "authorizes nor implements R6.",
        "reuses and locks the existing kernel setting",
        "ZoomWindow",
        "Spline×Spline",
        $PeriodicRisk,
        "all 98 scenarios",
        "G9U1 = IMPLEMENTATION CANDIDATE — PENDING AUTHOR REVIEW")
    ...
    Assert-Contains -RelativePath $EvidencePath -RequiredText @(
        ...
        "G9U0-R6",
        "G9U1",
        "DESIGN_CANDIDATE_PENDING_AUTHOR_REVIEW",
        $PeriodicRisk)
    ...
    Assert-NotContains -RelativePath $PromptPath -ForbiddenText @(
        "G9U1 = PASS", "implementationAuthorized = true")
...
try {
    ...
    $summary = @(
        ...
        "G9U0-R6 and G9U1 remain unimplemented and not authorized."
    )
