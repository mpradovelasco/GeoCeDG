...
| Estado actual | G7 y G8 `PASS`; G9P-R1, G9P, G9O1, G9A1, G9A2, G9A3, el track G9A, G9U0, G9U0-R1, G9X1, G9U0-R2, G9U0-R3, G9U0-R4, G9U0-R5, G9S1, G9U0-R6 y la planificación G10P `PASS — AUTHOR APPROVED`. R6 añade resolución inversa semántica y estado explícito de punto draggable en kernel, incluido cruce periódico bidireccional y path-independent, sin `Path`, render/pixel como autoridad ni frontend G9U1. Al no existir consumidor Desktop productivo R6, el autor acepta el test-host/API como superficie diagnóstica y difiere el smoke GUI a G9U1. El checkpoint G9U1 pre-R6 `857de662...` permanece protegido e inmutable; la rama sucesora reconcilia su diseño contra el contrato R6 publicado y ese diseño es `PASS — AUTHOR APPROVED`. G9U1 no se ha ejecutado y su implementación no está autorizada. R4 conserva determinismo actual > heurística de continuidad y el riesgo abierto `G9-R4-PERIODIC-QUARANTINE-NATIVE-ROUNDTRIP`. G9B/G9C no están autorizadas, G9U2 sigue bloqueada y ninguna implementación productiva G10 está autorizada; Locus V2 y la semántica espacial siguen experimentales y desactivadas por defecto |
| Última fase cerrada | G9U0-R6 — `PASS — AUTHOR APPROVED` |
| Última fase ejecutada | G9U0-R6 — `PASS — AUTHOR APPROVED`; `selfApproved=false`, `authorApproved=true`, `passClaimed=true`; `manualGuiSmoke=DEFERRED TO G9U1 BY DESIGN`; `kernelDiagnosticAcceptance=PASS` |
| Siguiente puerta | Tarea independiente de rendimiento/gobernanza de verificación y, posteriormente, decisión autoral separada sobre la autorización de implementación G9U1; G9U1 permanece sin ejecutar |
...
| G9U1 design | `PASS — AUTHOR APPROVED — POST-R6 RECONCILED` | Sucesor del checkpoint pre-R6 inmutable `857de6628489bda0b65a5ba5145e62ca0795fc32`: workspace Construction con manifiesto v2 candidato, 11 familias/18 clusters y matriz de escenarios validada. El Point tool consume el resolver tipado R6, usa chooser explícito ante múltiples preimages y falla cerrado ante none/unresolved/invalid/degenerate; drag conserva el mismo punto/ID y el cruce periódico usa el contrato R6. `Continuity = OFF`; markers de intersección siguen consumiendo tokens R4, separados de las preimages R6; create-one/create-selected/create-all y sesión persistente siguen prospectivos. La implementación sigue `NOT AUTHORIZED / NOT STARTED` |
...
**Estado del diseño:** `PASS — AUTHOR APPROVED — POST-R6 RECONCILED`

**Estado de implementación:** `NOT AUTHORIZED / NOT STARTED`
...
todavía exige una autorización autoral separada.
...
diseño está aprobado; la implementación sigue `UNEXECUTED / NOT AUTHORIZED` y
requiere una decisión autoral separada.
...
110 acciones estables y la autoridad post-R6 contiene exactamente 118 escenarios,
