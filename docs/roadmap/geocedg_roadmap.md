...
| Siguiente puerta | `PRE-G9B-R0` está `PASS — AUTHOR APPROVED` (decisión autoral de 2026-09-17). La aprobación cubre la caracterización, la disposición de deuda, `AD-R0-1`…`AD-R0-9` y el diseño escalonado revisado, e incorpora la extensión `R2-E0`/`R2-E1` de materialización por token exacto entre curvas semánticas decidida por el autor; `D1` se mantiene como capacidad intencionadamente no soportada y no se reescribe como deuda. La siguiente acción es una autorización autoral explícita de una fase concreta del track: `R1`, `R1-R1`, `R2`, `R2-E0`, `R2-E1`, `R3`, `R4`, `R5-A`, `R5-B`, `R6` y `R7` siguen `DESIGNED — NOT AUTHORIZED`. No se presupone que la siguiente implementación sea G9B. La publicación efectiva, el tag y cualquier release siguen requiriendo una disposición autoral separada. Contactos COMMERCIAL-A/OpenGeoProver, distribución comercial, marca figurativa, G9B, G9C, G9U2, G12 y G10 productivo permanecen no autorizados |
...
validados ni deben presentarse como plataformas soportadas. El perfil por
defecto del repositorio es `INTERNAL` y etiqueta sus outputs
`INTERNAL EVALUATION — NOT FOR REDISTRIBUTION`; el perfil `NC` etiqueta los
suyos `NON-COMMERCIAL DISTRIBUTION — PROFILE NC`.
...
- factor común y teclas configurables para zoom inmediato de entrada/salida
  anclado en el cursor vigente o en el centro real de la vista;
...
| G9 | Semántica espacial/proyecciones e integración pública Locus/Spline/producto | `IN PROGRESS`; G9U1 y track post-U1 cerrados, extensión pre-G9B en diseño, G9B/G9C no autorizadas y cierre global pendiente |
...
| G12 | Navegación, zoom y escalas extendidas | `PENDING`; slice A7 `PASS — AUTHOR APPROVED`, resto de G12 no autorizado |
...
| G9U0 | `PASS — AUTHOR APPROVED` | Superficie pública Locus V2 experimental, desactivada por defecto en su propio cierre y promovida después a default GeoCeDG por `PRE-G9B-P1`; 93 pruebas focales y repetición determinista sin fallos; candidato de 114 rutas preservado |
...
| G9X1 | `PASS — AUTHOR APPROVED` | DXF extendido externo al kernel; 62/62 escenarios focales y 10/10 regresiones G5, repetición determinista, sidecar condicional obligatorio y escritura pareada validados; experimental y default-off en su propio cierre, promovido después a default GeoCeDG por `PRE-G9B-P1` bajo su gate independiente; no implementa DXF `SPLINE` exacto |
...
| PRE-G9B-S1–S4 / D1 / P0 / P1 | `COMPLETE — AUTHOR APPROVED`; todas las puertas del track están `PASS — AUTHOR APPROVED`. La publicación de binarios, el tag y cualquier release siguen no autorizados | [Diseño pre-G9B](../architecture/pre_g9b_stabilization_deployability_design.md) aprobado en `1b7a7a43a3789d928f44d294364b159c14e6af59`, árbol `1700772eef774d79e6bd2c4d801476466ad996b6`: estabilización/DXF -> Text/property -> Text zoom -> tamaños de presentación -> licencias/assets/deployability -> promoción pública/1.0; orden autoral, no nueva dependencia semántica de G9B |
...
```text
TRACK_STATUS = COMPLETE — AUTHOR APPROVED
...
A7_RESEARCH_DESIGN = AUTHOR APPROVED
A7_PRODUCT_IMPLEMENTATION = PASS — AUTHOR APPROVED
...
| A7 | `PASS — AUTHOR APPROVED`; ownership G12 | `CLOSED AS BOUNDED G12 SLICE` | El candidato inicial `b249a62...` estableció cursor vigente/stale y centro no cuadrado; los sucesores `ee0f6acd...` y `ff45382...` conservaron los hallazgos de los smokes intermedios. El candidato final autor-aprobado `ed3f19a...` separa vigencia del cursor de estado interactivo, arma ZoomWindow sin temporización, sincroniza selección menú/toolbar y mantiene feedback en filas propias con editores expansibles. Conserva Zoom In/Out heredado, Escape, relación X/Y y factor común 10 con bindings independientes. La [matriz A7](../validation/post_g9u1_a7_navigation_design_matrix.md) mantiene fuera ZoomPrevious, FitSelection/FitLayer, vistas nombradas, escalas generales/impresión, 3D, Web y rediseño global. Capa view/workspace; viewport, cámara, DPI y navegación nunca determinan coordenadas, métrica, identidad ni proveniencia geométrica |
...
G9U1 PASS
  ...
  -> PRE-G9B-S1 stabilization/DXF (R1/R2 PASS — AUTHOR APPROVED)
  -> PRE-G9B-S2 construction Text/property correctness (PASS — AUTHOR APPROVED)
  ...
  -> PRE-G9B-P1 public surface / G9X1 default / PROFILE NC packaging
     (PASS — AUTHOR APPROVED; author smoke PASS)
  -> POST-P1 ROADMAP / TECHNICAL-DEBT DISPOSITION (planning only)
  -> separate later author decision on the next productive gate, G9B included
  -> G9C
  -> G9 global closeout
  -> G9U2
```
...
como slice de ejecución. Tras cerrar ese track, el autor pidió la extensión
pre-G9B descrita más abajo. Solo una decisión autoral posterior a S1/D1/P1 podrá
volver a seleccionar y autorizar el primer slice productivo G9B; aquí no se
selecciona ninguna familia ni se autoriza implementación.
...
#### POST-G9U1-A2 — política explícita de recorrido aprobada
...
El candidato `c0a018ab01c0b44a7ed1fb33010bf2481e30ac2a` fue aprobado
expresamente por el autor y publicado como fast-forward; `selfApproved=false`.
La matriz y los recibos del candidato se preservan como evidencia histórica de
esa aceptación.

#### POST-G9U1-A4 — posición de construcción y rollback

La [caracterización A4](../architecture/post_g9u1_a4_construction_position_rollback_characterization.md)
y su [matriz reproducible](../validation/post_g9u1_a4_construction_position_characterization_matrix.md)
separan identidad durable, DAG, orden de evaluación, `ceList`, índice mutable,
orden XML, presentación/navegación del Construction Protocol, referencia Java y
snapshots undo/redo. El autor aprobó investigación, diseño y P3 en
`b12d9ad04a766e893b120f3155250d5f1654c19b`, árbol
`930e58379dfe2e87e80d1ac81963636809274c41`; A4 no cambió producto y esa
decisión no autorizó por sí sola A3.

El candidato de diseño selecciona P3: una redefinición V2 compatible debe
preservar el intervalo procedimental relativo entre anchors supervivientes no
afectados, siempre dentro de los límites de predecesores/dependientes. El mismo
índice numérico no es identidad y la sola equivalencia topológica es
insuficiente: el host actual puede mover una salida compatible de nueva
instancia a través de un vecino independiente. El rollback existente sí
reconstruye el XML de entrada y con ello identidad durable, DAG y orden de
construcción exactos; puede reemplazar instancias Java y no restaura el paso de
navegación del Protocolo como estado semántico.

A3 recibió autorización separada y su candidato R1 reemplazante implementa el
mecanismo kernel acotado de captura/validación de anchors y recolocación mínima
forzada por DAG para la ruta compatible exitosa. No rediseña el rollback XML
existente. El Construction Protocol sigue siendo una
consecuencia de la construcción kernel y nunca un mecanismo de redefinición,
rollback o identidad.

#### POST-G9U1-A3-R1 — redefinición avanzada y fallback legacy explícito
...
candidato y evoluciona revisiones, revalidando bindings A1/A2/V2.

Los grupos multi-output se identifican por la relación completa
`stableOutputRole -> PersistentGeoId` de un único productor, nunca por ordinal;
su orden canónico solo hace determinista comparación/serialización. P3-R1 usa
exclusivamente evidencia precapturada, incluidos elementos GeoGebra ordinarios
como witnesses locales de transacción sin identidad durable. Mantiene el
intervalo anterior cuando es legal y, si el DAG lo impide, recoloca solo el
grupo hasta la cota legal más cercana, preservando el orden no afectado. P3-R1
es una enmienda/refinamiento explícito de P3 aprobada por el autor, no mera
equivalencia topológica: `destination = clamp(oldSlot,
requiredPredecessorBound, requiredDependentBound)` y nunca cualquier orden
topológico válido.

Fallo de `RETAIN` no autoriza `FRESH`. El preflight tipado solo ofrece fallback
legacy cuando el redefine host es válido, el proveedor clasifica reemplazo real
...
registra el slice Classic 5 `PASS — AUTHOR APPROVED`. La ruta compatible ejecuta solo
...
no admitidos o stale no ofrecen fallback; Web permanece fuera de alcance y la
autoridad semántica continúa en el kernel.

#### A6-OP1 — hardening de interpretación del exit code de `ripgrep`

La inspección A6 no encontró scripts/wrappers ejecutables del repositorio que
invoquen `rg` o interpreten su exit code. La regla queda codificada una sola vez
en la autoridad raíz de agentes para PowerShell y otros workstations:
...
No se duplicó la regla entre prompts ni se cambió tooling. A6-OP1 queda resuelto
como cambio documental/operacional sin efecto de fase productiva, geometría o
dependencia G9B.

### Track PRE-G9B — estabilización, deployability y promoción pública

**Estado:** track `COMPLETE — AUTHOR APPROVED`; S1, R1, R2, S2, S3, S4, S4-R1,
S4-R2, D1, P0 y P1 están `PASS — AUTHOR APPROVED`. D1 conserva como historia su
audit/disposition y su checkpoint de licensing framework/payload closure; aprobó
PROFILE NC para la composición auditada exacta sin crear ni publicar binario
alguno. P0 cierra los tres temas de presentación cerrados, la consolidación de
Preferences en `Layout & Presentation` y la versión canónica visible `1.0.0`, con
smoke autoral **PASS** y `selfApproved=false`. P1 está `PASS — AUTHOR APPROVED`
con smoke autoral **PASS**: promueve la superficie pública, deja PROFILE NC
técnicamente empaquetable y no publica nada. Generar un paquete NC no es
publicarlo: el tag, la release y cualquier distribución de binarios siguen
requiriendo una decisión autoral separada, y la distribución comercial sigue
bloqueada por términos externos. El cierre registra cuatro deudas abiertas en la
disposición post-P1 de más abajo.
...
|---|---|---|
| `PRE-G9B-S1` | reproducir/corregir DXF contra G5/G9X1 y estabilización acotada, separando los contratos UI no triviales | `PASS — AUTHOR APPROVED`; integra R1/R2, evidencia automática final y smoke autoral sin autorizar S2+ |
| `PRE-G9B-S1-R1` | población DXF geométrica tipada y consumo de componentes continuos certificados | `PASS — AUTHOR APPROVED`; `geocedg-dxf-geometric-2d/v1` preservado; evidencia en [informe S1](../validation/pre_g9b_s1_dxf_stabilization_candidate_report.md) y [caracterización R1](../architecture/pre_g9b_s1_periodic_locus_and_complete_population_characterization.md) |
| `PRE-G9B-S1-R2` | existencia Locus V2 y certificación de componentes continuos válidos en kernel compartido | `PASS — AUTHOR APPROVED`; [arquitectura](../architecture/pre_g9b_s1_r2_locus_existence_components_design.md), [spec normativa](../../geocedg/specs/locus/locus-v2-existence-components.md), [ADR 0027 aceptado](../adr/0027-locus-v2-existence-and-continuous-valid-components.md) y [matriz](../validation/pre_g9b_s1_r2_locus_existence_validation_matrix.md) |
| `PRE-G9B-S2` | corrección de transacción Text Properties y fuente de objeto de construcción hasta al menos 10 pt, con contratos internos separados | `PASS — AUTHOR APPROVED`; [informe de cierre](../validation/pre_g9b_s2_text_property_font_candidate_report.md); `selfApproved=false`, sin autorizar S3+ |
...
| `PRE-G9B-P0` | temas de presentación cerrados, consolidación de Preferences en `Layout & Presentation` y fijación de la versión canónica `1.0.0` | `PASS — AUTHOR APPROVED`; smoke autoral **PASS** (corrección de versión visible `1.0` -> `1.0.0` aplicada e integrada); candidato técnico `14b4eb135b090a412154a9b2fbfbef85f6705bc5`, tree `7eef19e2251361637b64377e72993311fd31301a`; PHASE `PRE-G9B-P0` `verification-d919002ce43b413e9172751205af4ac4` `ACCEPTED / COMPLETE`; [informe de candidato](../validation/pre_g9b_p0_presentation_theme_candidate_report.md) y [contrato normativo](../../geocedg/specs/ui/application-presentation-theme.md); `selfApproved=false`; no promociona package/distribution state ni autoriza P1 |
| `PRE-G9B-P1` | promoción de la superficie pública LocusV2/SplineV2 y del DXF extendido G9X1, compatibilidad Classic/archivos históricos, generalización del packaging y preparación de release PROFILE NC | `PASS — AUTHOR APPROVED`; smoke autoral **PASS**; candidato técnico `cac345ae2ba29bb983613f7fb66461b4d063c87b`, tree `71153c3b95b974225c16e52fd177501ec7537663`; PACKAGING NC `verification-938fbfcbbd514c58b12caca2653d10ac`, PACKAGING INTERNAL `verification-9e2f073dcb8847729255c5dc634ae553`, PHASE `verification-1f875e6221f34f0f8b6026ae5c093359` y FINAL `verification-13530e3b53634771b0796cabb9910c07`, todos `ACCEPTED / COMPLETE`; LocusV2/SplineV2 y `cedg.export.dxf.extended` pasan a default GeoCeDG de forma independiente; `--enableLocusV2` se conserva como override bidireccional; perfiles `INTERNAL`/`NC`/`COMMERCIAL` con COMMERCIAL fail-closed; set NC completo generado y verificado; [informe de candidato](../validation/pre_g9b_p1_public_surface_and_nc_packaging_candidate_report.md) y [matriz](../validation/pre_g9b_p1_validation_matrix.md); `selfApproved=false`; no publica, no etiqueta y no autoriza distribución comercial. Continuación correctiva acotada: legal bundle reconciliado a documentos generales profile-neutrales con la condición de distribución en el notice del perfil, evidencia D1 fechada preservada, identidad INTERNAL conservada y auditoría de la guía de usuario frente a los defaults promovidos. Deudas abiertas registradas en el cierre: `TD-P1-SEMANTIC-LENGTH-INTERSECTION` y `TD-P1-PACKAGING-SUMMARY` |
...
PACKAGING `verification-368265e284424ff788917717f271c28d` y STATIC
`verification-ef12293ff46d48cba5d7ea3c280fecd5` están `ACCEPTED / COMPLETE`
sobre `bd378db3…`. La asociación `.cedg` ya estaba correctamente materializada
en el MSI; el defecto era un falso negativo del verificador anterior, corregido
para decompilar el modelo WiX 5 y validar determinísticamente extensión, MIME,
ProgId, verbo open y destino `GeoCeDG.exe`.
...
resuelve cada deuda abierta y solo después qué gate productivo corresponde. No
se presupone que la siguiente implementación sea G9B.

| ID | Origen | Tipo | Estado |
|---|---|---|---|
| `TD-P1-SEMANTIC-LENGTH-INTERSECTION` | P1-D1, smoke autoral de P1 | kernel semantic / provenance / metric endpoint | `OBSERVED — REQUIRES CHARACTERIZATION/DESIGN` |
| `TD-P1-PACKAGING-SUMMARY` | P1-D2, smoke autoral de P1 | external tooling / reporting | `OBSERVED — SMALL BOUNDED MAINTENANCE` |
| `TD-VERIFY-RECEIPT-RECOVERY` | anterior a P1 | verification infrastructure | `PRE-EXISTING — NON-BLOCKING` |
| `TD-G9X1-HISTORICAL-PIN` | anterior a P1 | verification / historical authority reconciliation | `PRE-EXISTING — NON-BLOCKING` |

`TD-P1-SEMANTIC-LENGTH-INTERSECTION`. `Length(S,P,Q)` puede quedar indefinido
cuando `S` es una `SplineV2` y un extremo es un punto materializado desde una
intersección `SplineV2` × `SplineV2`. No es error numérico de la spline: la
resolución métrica actual admite puntos semánticos cuyo source es `S` y
occurrences únicas de puntos constructores, mientras que un punto de
`Intersect[R,"token"]` porta provenance de intersección y no es consumido por esa
resolución. La evidencia de intersección spline-pair ya conserva por lado
identidad de locus, revisión semántica, branch, componente, parámetro semántico y
evidencia de token/currentness. Cualquier solución futura debe consumir
provenance semántica explícita, nunca coordenadas ni proximidad, y caracterizar
al menos selección inequívoca del lado correspondiente a `S`, currentness,
token retention/reactivation, copy/reopen, autointersecciones, source A/B,
ambigüedad, redefine, persistencia y tests focales con regresiones. Requiere
caracterización y contrato semántico antes de elegir fase; no es un arreglo
rápido y no está autorizada su implementación.

`TD-P1-PACKAGING-SUMMARY`. El resumen de consola del build sigue emitiendo
`PUBLIC REDISTRIBUTION = BLOCKED PENDING LICENSE/ASSET APPROVAL` también para un
paquete NC correcto. Composición, legal bundle, manifest, installers e identidad
PROFILE NC son correctos; el defecto se limita al reporting. La corrección
debería derivar el resumen del distribution profile en lugar de mantener una
frase global histórica. Es candidata a una tarea pequeña e independiente
posterior a P1; no se corrigió en el cierre porque habría invalidado el candidato
ya sometido a FINAL y al smoke autoral.

`TD-VERIFY-RECEIPT-RECOVERY`. El flujo vigente produce evidencia FINAL
reproducible, pero no siempre emite un artifact separado conforme a
`verification-receipt.schema.json`. No deben fabricarse receipts ni hashes. No
bloquea P1 y no es defecto geométrico ni productivo; necesita una tarea acotada
de infraestructura de verificación.

`TD-G9X1-HISTORICAL-PIN`. `tools/agent/verify-g9x1-extended-dxf.ps1` conserva un
hash histórico cuya autoridad evolucionó legítimamente en `PRE-G9B-S1-R1`. La
reconciliación futura debe distinguir snapshot histórico, autoridad viva sucesora
y relación de supersesión o blob congelado, en lugar de sustituir el hash. No
reabre G9X1: `G9X1 = PASS — AUTHOR APPROVED` se mantiene.

El riesgo `G9-R4-PERIODIC-QUARANTINE-NATIVE-ROUNDTRIP` sigue `OPEN / TRACKED` y
requiere disposición autoral explícita antes del cierre global G9; esta
disposición post-P1 debe decidir también cómo se relaciona con las cuatro deudas
...
**Estado:** `PRE-G9B-R0` está `PASS — AUTHOR APPROVED` (decisión autoral de
2026-09-17, con un cambio de planificación requerido ya aplicado); `R1`, `R1-R1`,
`R2`, `R2-E0`, `R2-E1`, `R3`, `R4`, `R5-A`, `R5-B`, `R6` y `R7` están
`DESIGNED — NOT AUTHORIZED`. `selfApproved=false`.
...
#### Decisión autoral sobre D1 y extensión del track

La revisión autoral confirma la caracterización de R0 y, en particular, mantiene
`D1` como `INTENTIONALLY UNSUPPORTED CURRENT CAPABILITY`. **`D1` no es un defecto
y no se reescribe como deuda técnica.** El comportamiento rich-only actual de los
pares semánticos genéricos no soportados sigue siendo conforme a los contratos
aprobados vigentes.

El autor decide además que la materialización de puntos por token exacto entre
curvas semánticas es una capacidad importante de GeoCeDG y debe incorporarse
explícitamente al track `PRE-G9B-R` antes de su cierre. Esta decisión sustituye
**únicamente** la conclusión de planificación de R0 según la cual la extensión
quedaba fuera de `PRE-G9B-R`. No invalida la caracterización factual de `D1`, ni
los contratos históricos, ni la aprobación vigente de `G9S1`/`G9S1-R1`, ni
ninguna evidencia existente.

Deben mantenerse separadas dos afirmaciones:

| | Afirmación |
|---|---|
| **Hecho de producto vigente** | los pares semánticos genéricos no soportados siguen siendo rich-only |
| **Decisión autoral de roadmap** | `PRE-G9B-R` incluirá una extensión diseñada por separado para ampliar la materialización por token exacto hasta donde la prueba semántica lo permita |

`AD-R0-1 = AUTHOR DECIDED — INCLUDE LOCUS V2 PAIR MATERIALIZATION AS A
PRE-G9B-R CAPABILITY EXTENSION`. El objetivo es más amplio que el par mixto
observado: `R2-E0` debe investigar y, donde sea semánticamente defendible,
habilitar la materialización para `SplineV2 × SplineV2` certificado (línea base
de referencia aprobada), `SplineV2 × LocusV2` genérico, `LocusV2` genérico
`× SplineV2`, `LocusV2` genérico `× LocusV2` genérico, e imágenes y
composiciones semánticas soportadas de esas familias cuya provenance siga siendo
certificable. La elegibilidad debe basarse en capacidad y provenance semánticas
explícitas, nunca en el nombre de clase Java. `SeveralDefects.cedg` es fixture
objetivo obligatorio cuando `R2-E1` llegue a autorizarse.

Ni el ADR ni la especificación normativa de la extensión se crean aquí: son
salidas de `R2-E0` tras autorización separada. Este cierre **no autoriza** `E0`
ni `E1`.
...
| ID | Alcance | Estado |
|---|---|---|
| `PRE-G9B-R0` | caracterización, disposición de deuda y diseño escalonado | `PASS — AUTHOR APPROVED`; `DOCUMENTATION_STATUS_ONLY`; `selfApproved=false` |
...
| `PRE-G9B-R2-E0` | diseño de la extensión de materialización por token exacto para pares Locus V2 | `DESIGNED — NOT AUTHORIZED FOR EXECUTION`; tipo `SEMANTIC CAPABILITY EXTENSION DESIGN`; capa kernel compartido / contrato geométrico normativo; sin implementación productiva |
| `PRE-G9B-R2-E1` | implementación de la materialización por token exacto para pares Locus V2 | `DESIGNED — NOT AUTHORIZED`; ejecutable solo tras revisión y aprobación autoral del candidato exacto de ADR/especificación de `E0` |
...
Se adoptó la secuencia propuesta por el autor con cuatro cambios. El primero es
la decisión autoral de revisión: se insertan `R2-E0` y `R2-E1` entre `R2` y `R3`
para llevar la materialización generalizada por token exacto entre curvas
semánticas (`AD-R0-1`). Los otros tres están justificados por evidencia: `R2` se reduce a provenance del extremo métrico, porque `D1` es
...
#### Decisiones autorales (todas decididas el 2026-09-17)

`AD-R0-1` incluir la materialización de pares Locus V2 como extensión de
capacidad de `PRE-G9B-R` (`R2-E0` diseño, `R2-E1` implementación), con alcance
más amplio que el par mixto observado. `AD-R0-2` resolver `checker_identity_hash`,
`input_identity_hash` y `accepted_profiles` **dentro de `R1`**, reutilizando la
semántica de aceptación existente, sin crear un segundo sistema de aceptación y
sin fabricar hashes. `AD-R0-3` planificar el cierre del riesgo R4 en `R1-R1` con
evidencia real de ciclo de vida nativo `.cedg`, evitando ampliar la API del
kernel solo para habilitar un test cuando la evidencia durable basta. `AD-R0-4`
adoptar la presentación canónica en inglés de los nombres de comando,
conservando los nombres localizados como alias de compatibilidad en la entrada
ordinaria donde la compatibilidad vigente lo permita, sin cambiar XML ni
serialización y manteniendo GGBScript canónico inglés/interno; se formaliza en el
ADR de compatibilidad de `R5-B`. `AD-R0-5` corregir el tooltip inválido de
`SplineV2` dentro de `R4`, no como microfase independiente. `AD-R0-6` `R3` se
planifica como fase integrada, sin migración silenciosa y anticipada de firmas
persistidas históricas: si la proyección canónica exige migración, se diseña
explícitamente y se detiene para revisión autoral antes de implementarla.
`AD-R0-7` no crear por ahora un segundo ledger global de deuda técnica.
`AD-R0-8` el Desktop puede construir la forma cerrada canónica solo mediante una
acción de usuario explícita de forma cerrada, y puede sintetizar el miembro
primero/último repetido que exige la representación del kernel; nunca inferir el
cierre por proximidad de coordenadas, coincidencia visual o un clic repetido
ambiguo. `AD-R0-9` autorizar, **después del cierre de R0**, una tarea separada de
capa de gobierno que añada stubs de prompt, que deben permanecer
`PROPOSED / UNEXECUTED / NOT AUTHORIZED` hasta su autorización separada y cuyo
inventario debe incluir `R2-E0` y `R2-E1`; `PRE-G9B-R0` no los creó porque
`CLAUDE.md` prohíbe editar `.github/prompts/**` como efecto colateral de otra
tarea.

La aprobación autoral de `PRE-G9B-R0` cubre la caracterización, la disposición
de deuda, `AD-R0-1`…`AD-R0-9` y el diseño escalonado revisado incluidos `R2-E0`
y `R2-E1`. **No autoriza la ejecución** de `R1`, `R1-R1`, `R2`, `R2-E0`, `R2-E1`,
`R3`–`R7`, G9B, G9C, G9U2, G10 productivo, G12 adicional, DXF `SPLINE` exacto,
PROFILE COMMERCIAL, distribución comercial, publicación de binarios, tag, release
ni contactos con GeoGebra/OpenGeoProver. Registrar `R2-E0`/`R2-E1` en el roadmap
no inicia ninguna de las dos fases.
...
Tampoco A1–A7 ni PRE-G9B-S1–S4/D1/P1 son dependencias duras de G9B: su precedencia
es exclusivamente el orden de ejecución seleccionado por el autor. Cada familia —recta, segmento,
rayo, vector, plano, circunferencia, cónica o curva espacial soportada— requerirá
su stage canónico individualmente aprobado. Tras el track pre-G9B, otra decisión
...
**Estado:** `PENDING`; slice A7 `PASS — AUTHOR APPROVED`; resto de G12 no
autorizado
...
sin convertirlo en dependencia semántica G9. Su
[diseño autor-aprobado](../architecture/post_g9u1_a7_navigation_design.md)
reutiliza el `ZoomWindow` G9U1 y el transform de vista heredado. El smoke del
candidato inicial `b249a62...` exigió preservar relación X/Y en ZoomWindow,
cancelación Escape, validación corregible sin cerrar el diálogo y dos acciones
configurables de zoom por factor; no reabrió el resto de G12.
El sucesor `ee0f6acd...` pasó la verificación automatizada, pero un segundo smoke
autoral encontró dos defectos acotados: ausencia de feedback inmediato de
validación y activación no operativa desde los controles reales de menú/toolbar.
El sucesor `ff45382...` corrigió y verificó esos dos puntos, pero el smoke repetido
halló la autocancelación tardía por pérdida de foco y el colapso del layout ante
mensajes largos. El candidato final `ed3f19a095ce661307dbd6d9a24e70f6569cc4cf`,
árbol `9b4385b7c7d7da4cf381068fd0c245d66be9a8c8`, conserva el contrato, elimina la dependencia
temporal, mantiene una herramienta interactiva persistente y estabiliza el layout
sin ampliar G12. Su PHASE final fue `ACCEPTED / COMPLETE` con 91 pruebas y cero
fallos, errores, omisiones o diagnósticos; el smoke interactivo final fue ejecutado
y aceptado por el autor. Esa evidencia separada cierra A7, no el resto de G12.

**Trabajo**

- zoom centrado en cursor, ZoomWindow explícito y zoom de entrada/salida por
  factor con teclas configurables;
