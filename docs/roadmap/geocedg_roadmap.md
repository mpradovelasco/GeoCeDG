...
| Estado actual | G7 y G8 `PASS`; G9P-R1, G9P, G9O1, G9A1–G9A3, el track G9A, G9U0 y sus refinamientos R1–R6, G9X1, G9S1, G9S1-R1, G9U1 y POST-G9U1-A6, A1, A2, A3, A3-FRONTEND, A5 y A7 `PASS — AUTHOR APPROVED`. POST-G9U1-A4 tiene investigación/diseño completos y `AUTHOR APPROVED`; P3-R1 es su refinamiento autor-aprobado. El track post-G9U1 está `COMPLETE — AUTHOR APPROVED`. La extensión pre-G9B está `DESIGN — AUTHOR APPROVED`; `PRE-G9B-S1`, R1, R2, `PRE-G9B-S2`, `PRE-G9B-S3`, `PRE-G9B-S4`, S4-R1 y S4-R2 están `PASS — AUTHOR APPROVED`. El checkpoint de licensing framework/payload closure de `PRE-G9B-D1` está `COMPLETE — AUTHOR APPROVED`: 39/39 JAR externos aplicables y 46/46 fonts quedan trazados para PROFILE NC, con PACKAGING/STATIC aceptados. La fase D1 permanece `OPEN`, sin claim de PASS; `selfApproved=false` y `publicRedistributionAllowed=false` hasta revisión legal/fallo autoral final. P1 conserva su orden autoral y no está autorizado. G9B/G9C permanecen diseñadas y no autorizadas; G9U2 sigue bloqueada por la aprobación global G9; G10P es solo planificación aprobada y ninguna implementación productiva G10 está autorizada. El riesgo `G9-R4-PERIODIC-QUARANTINE-NATIVE-ROUNDTRIP` continúa abierto para disposición antes del cierre global G9. Locus V2 y la semántica espacial siguen experimentales y desactivadas por defecto |
| Última fase cerrada | PRE-G9B-S4, R1 y R2 — `PASS — AUTHOR APPROVED`; candidato técnico `eb6af9799eec70459485b13fe1c9cadd394e8dca`, árbol `f71c34fff34fcff169a1302eb08e879bb247802d`; smoke autoral `PASS`; `selfApproved=false` |
...
| Siguiente puerta | revisión profesional final del expediente PROFILE NC y disposición autoral separada sobre cierre de fase/release; el checkpoint documental ya está publicado/aceptado, pero HD-03/04 siguen diferidos y cualquier contacto o release requiere nueva autorización; P1, G9B, G9C, G9U2, más G12 y G10 productivo permanecen no autorizados |
...
G9U1 PASS
  ...
  -> PRE-G9B-D1 licensing/assets/deployability (DESIGN CANDIDATE; NOT AUTHORIZED)
...
| `PRE-G9B-D1` | inventario, investigación primaria, disposición humana, remediación autorizada y verificación de deployability | `LICENSING FRAMEWORK / PAYLOAD CLOSURE CHECKPOINT = COMPLETE — AUTHOR APPROVED`; candidato técnico `bd378db3279051410208c084ae51eac09244ced6`, tree `fe28dd24d1fd7d98fa89257233d3d9aa41d283aa`; 39/39 JAR externos y 46/46 fonts trazados para PROFILE NC; la fase D1 sigue `OPEN — PASS NOT CLAIMED`, pendiente de revisión profesional y disposición final; `selfApproved=false`, `publicRedistributionAllowed=false` |
...
El checkpoint D1 vigente está documentado en el [informe de cierre de payload](../validation/pre_g9b_d1_six_payload_closure_continuation.md). Conserva y enlaza el [audit factual](../validation/pre_g9b_d1_licensing_assets_deployability_research_audit.md), el [informe de licensing/disposition y remediación acotada](../validation/pre_g9b_d1_licensing_disposition_and_bounded_remediation_candidate.md), la [matriz machine-readable resuelta](../../geocedg/validation/pre-g9b-d1/component-disposition.json), el [docket humano](../validation/pre_g9b_d1_human_decision_docket.md), el [plan de remediación](../validation/pre_g9b_d1_remediation_plan_candidate.md), la [matriz viva de componentes](../licensing/component-matrix.md), el [asset manifest](../../geocedg/resources/assets-manifest.yml) y el [source-access manifest](../../geocedg/resources/source-access-manifest.json).
