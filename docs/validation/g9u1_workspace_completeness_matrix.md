...
| Stable action ID | Target | Placement | Disposition | Gap / prerequisite |
|---|---|---|---|---|
| `construction.point` | `MODE_POINT(1)` | toolbar | MUST | add stroke-only semantic LocusV2/SplineV2 selection and exact preimage contract |
| `construction.point-on-object` | `MODE_POINT_ON_OBJECT(501)` | toolbar | MUST | same semantic-curve contract; no generic `Path` conformance |

Both actions retain ordinary Classic operands. GeoCeDG semantic-curve placement
requires the approved feature and must fail closed on unresolved preimage
ambiguity. Dragging stores semantic branch/address, not Cartesian proximity.
...
SplineV2×SplineV2 remains rich-only without symmetric certified identity:
inspectable evidence, zero selectable markers, and zero materialization actions.
...
| `semantic.locus-v2.point-explicit` | `MODE_LOCUS_V2_POINT(134)` | overflow/menu | MUST | retained explicit helper; ordinary Point also gains semantic placement |
...
Transformed objects get new identity/tokens; no source-token reuse.
...
| Semantic Point placement/drag | MUST | bind exact branch/address/preimage; ambiguity requires explicit disambiguation or failure; no generic `Path` |
...
| SplineV2×SplineV2 materialization | OUT OF SCOPE / NEW KERNEL | rich-only until symmetric exact token identity is independently approved |

Mandatory entry authority includes clean PASS-tagged G9U0/R1/R2/R3/R4/R5 and
G9S1 descendants, ADRs 0012/0016/0017/0018, the current normative workspace,
document, Locus, transformation, spline, identity, and persistence contracts,
and a still-explicit disposition of the R4 periodic risk.
...
| SplineV2×SplineV2 markers/materialization | NEW KERNEL | no symmetric certified selector/token authority |
...
- every required future scenario, including the actual periodic-quarantine
  native round trip, is represented in the G9U1 validation matrix.

## 9. Terminal governance state

```text
G9U1 =
DESIGN CANDIDATE — PENDING AUTHOR REVIEW
