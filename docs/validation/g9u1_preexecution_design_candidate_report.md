# G9U1 post-G9S1-R1 reconciliation candidate

- Approved design baseline: PASS — AUTHOR APPROVED, POST-R6 RECONCILED.
- Current reconciliation: POST-R1 RECONCILED; NO_MATERIAL_NOVELTY.
- Conditional author implementation authorization: SATISFIED.
- Implementation: AUTHORIZED / NOT STARTED at the planning freeze.
- `implementationStarted = false`; `implementationAuthorized = true`;
  `selfApproved = false`; `authorApprovedDesign = true`;
  `authorApprovedImplementation = false`; `passClaimedImplementation = false`.
- Manual author smoke: PENDING.

## Current published source authority and import provenance

Clean entry main and successor branch base:
`f8a21a087234b18fc13741a0ac2baf80608e9022`.
Planning/implementation successor branch:
`codex/g9u1-construction-workspace-after-r1`.
R1 reviewed technical commit:
`a38d4fcde846fc97c51abc8d958de6998302c436`.
R1 approved closeout/tag peel:
`af459d856f1cdc384805f3035203acce8e6f6104`;
`geocedg-g9s1-r1-pass` is published. The later operational main does not
replace the R1 phase-tag target.

Protected approved post-R6 design:
`00982e7e148a634cd57ed928f322774df267d5e3`,
`feature/g9u1-construction-workspace-planning-after-r6`.
Its exact prompt remains unmodified:
...
The older checkpoint `857de6628489bda0b65a5ba5145e62ca0795fc32` and
post-G9S1 prompt hash
`2319df211f5ea17880b7041844122afca0f2ddced4c6db1fabddce0d53dfa322`
also remain unchanged.

Import method: no cherry-pick, rebase or merge. Fifteen non-overlapping paths
were imported through explicit patches and independently matched all fifteen
approved Git blob identities before any reconciliation delta. The roadmap,
G9 traceability and specs index were reconciled by the approved planning diff
and bounded semantic merges, preserving R1/operational descendants and historical
failed/candidate evidence. The old protected refs were not moved.

Canonical successor:
`.github/prompts/tasks/g9u1-construction-workspace-after-g9s1-r1.prompt.md`.
Current canonical-LF SHA-256:
`c9b56474fe22368bffaee7417511a3237cbde0f9e31d777b36532232b61f1451`.
No byte identity with the intentionally superseded design is claimed.

## Bounded R1 delta and complete gap audit

Use [ADR 0021](../adr/0021-spline-pair-singleton-germ-materialization.md)
and the [published R1 specification](../../geocedg/specs/curves/spline-v2-pair-materialization.md),
not a generic claim that all spline pair roots now materialize.

The exact subset is `pair-singleton-transverse-germ/v1`: structural interval
existence/uniqueness plus transversality and current unique canonical germ slot
are mandatory. Several distinct roots, including opposite germs in one span
pair, may each be eligible. A `+,+,-` group leaves the two positive-germ roots
rich-only while the unique negative-germ root can be admissible. Uncertified,
same-germ ambiguous, tangent/multiple-contact, overlap, monodromic, stale or
budget-unresolved candidates are never promoted by the frontend.
Local admissibility does not imply global completeness.

Operand reversal preserves canonical pair/slot evidence, **not equality of
opaque tokens from independently constructed rich-result owners**. R1's
`reversedCallerOperandsHaveEquivalentCurrentSlotEvidence` regression
explicitly distinguishes those owner namespaces. The UI reuses an exact token
only in its owning rich result, with no marker-position or R6 inverse fallback.

The current source audits found all 66 referenced upstream mode IDs, the actual
R6 create/move seams, rich-result/token materialization authority and the G9A
explicit-target atomic compatible-redefine seam. Existing preview-evaluates and
blur-commits behavior is an authorized ordinary frontend repair, not missing
kernel semantics. Schema-v1 to schema-v2 is a single-catalog migration;
declarative menu placement remains within that same authority. No new kernel
API, persistence identity field, geometric selector, family or Classic semantic
change is needed. Missing author branding bitmaps use existing text/default
fallback; no substitute artwork is authorized.

The Point-tool R6 system and pair-intersection R1 system remain separate.
All approved stroke-only selection, EN/ES, Continuity OFF, profile, Algebra
preview/commit, GGBScript, help, navigation/ZoomWindow, accessibility, native
document, compound undo and persistent-inspector requirements are retained.

## Counts, scope and validation plan

11 professional families; 18 operational clusters; **110 stable actions**.
The obsolete deferred pair-materialization capability classification is removed,
not replaced by a redundant action. **138 unique scenarios** = all 118 approved
rows plus 20 `U1-PAIR-01`–`U1-PAIR-20` frontend-consumption rows.
The revised existing `U1-S1-06` now states the R1-certified boundary.
The R1 scientific numerical suite remains kernel authority and is not duplicated
as a new frontend solver suite.

Planning validation is static only: exact 19-path inventory, zero productive
source/live-profile paths, 18 canonical hash entries, both protected prompt
hashes, links, action/matrix/scenario completeness, actual source API and phase
prerequisites, empty index and diff checks. Two equivalent planning executions
are required; results remain PENDING until their saved logs pass.
No PHASE/COMPOSED/FULL or productive G9U1 tests ran as planning validation.

## Risk and candidate boundary

`G9-R4-PERIODIC-QUARANTINE-NATIVE-ROUNDTRIP = OPEN / TRACKED`.
G9U1 will execute its native lifecycle experiment. The current explicit author
disposition permits this implementation candidate to continue if the experiment
is inconclusive without a demonstrated kernel/persistence violation. A real
contract failure requires STOP. Global G9 closeout still requires disposition.

The source/API/profile audit finds no material novelty. The latest author
instruction therefore authorizes productive implementation after this planning
gate, without a redundant administrative confirmation. It does not approve the
future implementation or author smoke. G9B/G9C, G9U2 and productive G10 remain
unauthorized/blocked as before.

## Exact planning inventory

1. `.github/prompts/tasks/g9u1-construction-workspace-after-g9s1-r1.prompt.md`
2. `.github/prompts/tasks/g9u1-construction-workspace-after-g9s1.prompt.md`
3. `.github/prompts/tasks/g9u1-construction-workspace-after-g9u0-r6.prompt.md`
4. `docs/architecture/cedg_workspace_architecture.md`
5. `docs/roadmap/geocedg_roadmap.md`
6. `docs/validation/g9_documentation_bundle_traceability.md`
7. `docs/validation/g9_public_workspace_validation_matrix.md`
8. `docs/validation/g9u1_command_tool_consistency_matrix.md`
9. `docs/validation/g9u1_preexecution_design_candidate_report.md`
10. `docs/validation/g9u1_workspace_completeness_matrix.md`
11. `geocedg/specs/README.md`
12. `geocedg/specs/ui/application-profile-v2.candidate.schema.json`
13. `geocedg/specs/ui/application-profile-v2.candidate.yml`
14. `geocedg/specs/ui/cedg-workspaces.md`
15. `geocedg/specs/ui/g9u1-construction-interaction.md`
16. `geocedg/validation/g9u1/g9u1-preexecution-design-evidence.json`
17. `geocedg/validation/g9u1/g9u1-preexecution-design-evidence.sha256`
18. `geocedg/validation/g9u1/g9u1-preexecution-scenarios.json`
19. `tools/agent/verify-g9u1-preexecution-design.ps1`

## Historical approved post-R6 report — preserved provenance

The text below records the former checkpoint's approval, statuses, path/scenario
counts and execution hashes. Its NOT AUTHORIZED statements and 118-row results
belong to that historical checkpoint; they do not override the current explicit
conditional authorization or claim execution on this successor. Source contracts
changed only through the bounded post-R1 reconciliation described above.
