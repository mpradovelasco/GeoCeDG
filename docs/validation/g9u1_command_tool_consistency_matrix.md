...
- Canonical future prompt:
  [`g9u1-construction-workspace-after-g9s1-r1.prompt.md`](../../.github/prompts/tasks/g9u1-construction-workspace-after-g9s1-r1.prompt.md)
...
| certified structural spline pair intersection | inspect / materialize eligible exact tokens | `Intersect(S1,S2)` | current R1 local certificate and singleton-germ selector | shared rich result with individually admissible tokens; others rich-only | existing V2 policy | existing one/selected/all actions; source-swap equivalent slots, owner-specific opaque tokens | U1-S1-06, U1-PAIR-01..20 |
...
4. Use the ordinary Point tool to click the stroke of a straight Locus V2 and a
   ...
   clicking only inside a closed curve does not select that semantic curve or
   create a semantic point on it. Ordinary empty-space Point-tool behavior
   remains inherited and may create an ordinary free point.
...
11. Inspect a certified spline pair and materialize one/selected/all eligible
   tokens; check same-span opposite-germ roots, operand reversal and owner
   namespaces. Also inspect same-germ, tangent/multiple-contact and unresolved
   controls: readable diagnostics but no selectable exact-token marker/action.
...
13. Test auto-materialization OFF and explicitly ON. Confirm ON groups the
   rich query and opted-in points into one visible compound undo/redo step for
   that Enter/tool confirmation; later explicit inspector creation has its own
   atomic undo step. Later recompute/root appearance
