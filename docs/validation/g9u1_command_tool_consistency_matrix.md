...
- Status: **PROSPECTIVE DESIGN AUTHORITY — G9U1 NOT AUTHORIZED**
- Phase: G9U1 — CeDG Construction workspace
- Required predecessors: G9U0, G9U0-R1, G9X1, G9U0-R2, G9U0-R3,
  G9U0-R4, G9U0-R5 and G9S1 sealed `PASS — AUTHOR APPROVED`
...
| F02 | Inspect and construct | Move, point, point-on-object, Algebra/Protocol/Properties toggles, undo/redo, object and rich-result inspection | stable focus/selection grammar and normal kernel transactions |
...
| F07 | Locus V2 and semantic Spline V2 | Locus V2 creation, Spline V2 public forms, semantic Point-on-Locus and semantic inspection | new semantic `GeoLocusV2` objects only; no sampled/render authority |
...
| semantic Point-on-Locus | create semantic point | approved `Point` semantic-address authority | source plus exact branch/component/address inputs | ordinary `GeoPoint` with semantic parentage | V2 gate for a new operation | Inspect and construct / Locus group; not token materialization | U1-W15 |
...
4. Transform one spline and use semantic Point, total/partial length and one
   supported one-sided intersection.
5. Select the rich result. Confirm only the active result shows current
   admissible markers. Open the inspector by keyboard.
6. In one inspector session create one point, several selected points and all
...
7. Inspect a supported spline×spline rich-only result. Confirm its evidence is
   readable but it offers no selectable marker or create-one/selected/all
   action.
8. Exercise compatible free-input `k=0.25`, undo/redo, `k=0` and recovery.
   Confirm the compatible numeric identity graph is preserved and the collapsed
   image creates no fabricated isolated marker.
9. Test auto-materialization OFF and explicitly ON. Confirm ON creates one
   ...
   reactivate.
10. Save/reopen `.cedg`; confirm workspace state remains presentation-only and
    semantic points/tokens/recompute persist.
11. Launch the separate Classic diagnostic route. Confirm Classic `Spline` and
...
12. Review topbar/startup branding roles, contrast, keyboard focus and normal/
